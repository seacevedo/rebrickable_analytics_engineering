        ...
        create or replace transient table REBRICKABLE_DB.REBRICKABLE_SCHEMA_marts.dim_theme
         as
...
dim_theme AS (
    SELECT md5(cast(coalesce(cast(theme_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(parent_theme as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(child_theme as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) AS theme_dim_id,
    theme_id,
    parent_theme,
    child_theme
    FROM union_theme
